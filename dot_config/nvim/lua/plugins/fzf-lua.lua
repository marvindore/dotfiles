vim.pack.add({
...

	{
		...
		data = {
			...
			keys = {
        { lhs = "<localleader>fr", rhs = "<cmd>lua require('fzf-lua').files({ cwd = vim.fn.expand('~/.local/share/chezmoi/runbook') })<CR>", mode = "n", desc = "RunBook find"},
        { lhs = "<LocalLeader>fR", rhs = "<cmd>lua require('fzf-lua').live_grep({ cwd = vim.fn.expand('~/.local/share/chezmoi/runbook') })<CR>", mode = "n", desc = "RunBook grep"},
        { lhs = "<LocalLeader>fc", rhs = "<cmd>lua require('fzf-lua').files({ cwd = vim.fn.expand('~/.local/share/chezmoi/dot_config') })<CR>", mode = "n", desc = "Config find"},
        { lhs = "<LocalLeader>fC", rhs = "<cmd>lua require('fzf-lua').live_grep({ cwd = vim.fn.expand('~/.local/share/chezmoi/dot_config') })<CR>", mode = "n", desc = "Config grep"},
        { lhs = "<LocalLeader>fn", rhs = "<cmd>lua require('fzf-lua').files({ cwd = vim.fn.expand('~/notesplus') })<CR>", mode = "n", desc = "NotesPlus find"},
        { lhs = "<LocalLeader>fN", rhs = "<cmd>lua require('fzf-lua').live_grep({ cwd = vim.fn.expand('~/notesplus') })<CR>", mode = "n", desc = "NotesPlus grep"},
				{ lhs = "<LocalLeader>fo", rhs = ":lua require('fzf-lua').oldfiles()<CR>", mode = "n", desc =  "Fzf open old files" },
